...
// Get by id_rol model
export async function getRolByIdModel(id_rol: number): Promise<NewRol | null> {
  const rol = await knexTableName().where({ id_rol }).first();
  return rol || null;
}

// Create rol
export const createRolModel = async (rol: NewRol): Promise<NewRol> => {
...
export async function deleteRolModel(id_rol: number): Promise<NewRol | null> {
  const updated_at = new Date();
  const [updatedRol] = await knexTableName()
    .where({ id_rol })
