...
export const getSubcategoryByIdService = async (
    ...
    try {
        const subcategory = await SubcategoryModel.getSubcategoryByIdModel(id_subcategory);
        return subcategory;
    } catch (error) {
        logger.error("error fetching subcategoryd", error);
        throw error;
...
export async function deleteSubcategoryService(id_subcategory: string): Promise<NewSubcategory | null> {

    const existing = await SubcategoryModel.getSubcategoryByIdModel(id_subcategory);
    if (!existing) return null;

    const deactivatedSubcategory = await SubcategoryModel.deleteSubcategoryModel(id_subcategory);
