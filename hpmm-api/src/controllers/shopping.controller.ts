...
export const UpdateShoppingController = asyncWrapper(
    async (req: Request, res: Response): Promise<void> => {
        const id_shopping = (req.params.id || "").trim();
        const { id_scompra, id_vendedor, fecha_compra, total,estado } = req.body;
        const shopping = await ShoppingService.updateShoppingService(id_shopping, id_scompra, id_vendedor, fecha_compra, total,estado);
        if (!shopping) {
            res.status(404).json({ msg: "Compra no encontrada" });
            return;
        }
