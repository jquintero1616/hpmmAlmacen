...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable("employes", table => {
    table.uuid("id_user").notNullable().primary();
    ...
    table.foreign("id_units").references("id_units").inTable("units").onDelete("SET NULL");
    table.foreign("id_subdireccion").references("id_subdireccion").inTable("subdireccion").onDelete("SET NULL");
    table.foreign("id_direction").references("id_direction").inTable("direction").onDelete("SET NULL");
  });
}
